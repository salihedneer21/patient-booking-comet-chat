...
export default function AdminPatientsPage() {
  ...
  const createPatient = useMutation(api.modules.telehealth.superAdmin.createPatient);
  const togglePatientStatus = useMutation(api.modules.telehealth.superAdmin.togglePatientStatus);
  const deleteUser = useMutation(api.modules.telehealth.superAdmin.deleteUser);

  if (userLoading || patientsData === undefined) {
    return <FullPageSpinner />;
  }

  const filteredPatients = (patientsData || []).filter((item) => {
    const searchLower = searchQuery.toLowerCase();
    const fullName = item.intake ? `${item.intake.firstName} ${item.intake.lastName}` : item.user.name || "";
    return (
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div className="flex-1">
          ...
          <p className="text-muted-foreground">
            {patientsData?.length || 0} patient{(patientsData?.length || 0) !== 1 ? "s" : ""} registered
          </p>
        </div>
        <Button
          onClick={() => {
        ...
        <CardHeader>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <CardDescription>
                Manage patient accounts ({patientsData?.length || 0} total)
              </CardDescription>
            </div>
              ...
              {filteredPatients.map((item) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => navigate(`/admin/patients/${item.user._id}`)}
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {item.intake && (
                            <Badge variant={item.intake.intakeCompleted ? "default" : "outline"}>
                              {item.intake.intakeCompleted ? "Intake Complete" : "Intake Pending"}
                            </Badge>
                          )}
                        </div>
                        <div className="flex items-center gap-4 text-sm text-muted-foreground mt-1">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <span>
                              Joined {format(new Date(item.user.createdAt), "MMM d, yyyy")}
                            </span>
                          </div>
                        </div>
                      </div>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem
                        onClick={(e) => {
                          e.preventDefault();
                          e.stopPropagation();
                          navigate(`/admin/patients/${item.user._id}`);
                        }}
                      >
                        View Details
                      </DropdownMenuItem>
                      <DropdownMenuItem
                        onClick={async (e) => {
                          ...
                          e.stopPropagation();
                          try {
                            await togglePatientStatus({ userId: item.user._id });
                            toast.success(
                              item.user.isActive ? "Patient deactivated" : "Patient activated",
                            );
                          } catch {
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              appointments, encounters, and medical records.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Creates a new patient account and generates a temporary password.
            </DialogDescription>
          </DialogHeader>
